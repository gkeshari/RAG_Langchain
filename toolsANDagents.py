...
def user_input(user_question):
    try:
        ...
        prompt = f"""
        Use the following context to answer the question concisely. If the answer is not in the context, say "I don't have enough information from the documents to answer this question."
...
def web_search(query):
    ...
    try:
        results = search.run(query)
        # Process the results to get a concise answer
        model = ChatGoogleGenerativeAI(model="gemini-pro", temperature=0.3)
        prompt = f"Based on the following search results, provide a brief and concise answer to the question: '{query}'\n\nSearch results: {results}"
        response = model.invoke(prompt)
        return response.content
    except Exception as e:
        logger.error(f"Error in web search: {str(e)}")
...
def process_question(question):
    document_answer = user_input(question)
    if document_answer is None or "I don't have enough information" in document_answer:
...
def main():
    ...
    other_docs = st.sidebar.file_uploader("Upload Other Document Files", accept_multiple_files=True, type=['docx', 'txt'])
    ...
    if st.sidebar.button("Process Documents"):
        with st.spinner("Processing documents..."):
            ...
            if raw_text:
                ...
                st.sidebar.warning("No text content found in the uploaded documents.")
